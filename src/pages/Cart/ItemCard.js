...
const ItemCard = ({ item }) => {
  ...
  const prods = useSelector((state)=> state.mainReducer.products)

  useEffect(() => {
    const prod = prods.find((product) => product.id === item.id)
    if (prod) {
  ...
  return (
    <div className="w-full grid grid-cols-5 mb-4 border py-2 rounded-3xl hover:shadow-lg">
      <div className="flex col-span-5 mdl:col-span-2 items-center gap-4 ml-4">
        <img className="w-32 h-32 rounded-3xl" src={item.img} alt="productImage" />
